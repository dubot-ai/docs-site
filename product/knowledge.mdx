...
<Frame caption="Monitor a crawl while discovered pages are added to the workspace knowledge corpus.">
  <img
...
Accepted crawls and uploads automatically update the corpus used by existing experiences. There
is no article-level Publish action. The experience's saved or published configuration controls
its retrieval scope. See [when article changes become available](/guides/add-knowledge#when-article-changes-become-available).
...
| Resource Center search | Searches the delivered corpus and returns article cards; raw search text is not retained as analytics content |
