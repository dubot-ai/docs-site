...
<Steps>
  ...
  <Step title="Resolve the allowed knowledge and tools">
    The active wizard supplies instructions and capabilities. Dubot narrows knowledge to the
    configured scope and actions to the published, enabled set that the host currently supports.
  </Step>
  <Step title="Answer, guide, or act">
    Dubot can respond in conversation, launch Guidance against the current interface, collect
    structured input, or request an approved action.
  </Step>
...
knowledge, Guidance, Resource Center content, and brand settings there. The browser SDK is the
runtime plane inside the customer product.
...
- Session API destinations come from the published action registry, not from model output.
