...
<CardGroup cols={3}>
  ...
  <Card title="Crawl" icon="radar">
    A committed logged-in browser pass explores the product and contributes pages, semantic targets, available behavior, and account variation to the Product Map.
  </Card>
...
| Crawl | Yes | Product Map evidence, candidate journeys, and proposed updates | Committed capability |
...
## How crawling supports change

The committed crawl model is designed to refresh the [Product Map](/concepts/product-map), not silently rewrite production journeys. Configurable recrawls can identify changed pages or controls, lower the health of affected journeys, and propose updates for review.

A common product change should improve the shared map. An account-specific difference should update the account overlay rather than create a copied journey.
