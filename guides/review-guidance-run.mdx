...
<Steps>
  <Step title="Edit the guide details">
    Rewrite the title, goal and start condition in general terms, then add one trigger phrase per
    line. The generated draft reflects the run, so its title can name a recovery step and its start
    condition can contain page IDs. Trigger phrases stay off until the guide is published. See
    [Write guides that work](/guides/write-guidance-guides) for what to check in each field.
  </Step>
...
<Frame caption="A reviewed guide in Test waits on the highlighted customer-owned step.">
  <img
    ...
    alt="A reviewed guide in Test waiting for the customer to complete a highlighted step"
  />
</Frame>
